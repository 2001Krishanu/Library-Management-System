                        ...
                        while ($row=mysqli_fetch_assoc($result)){
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="profile_delete.php?account_delete=<?= base64_encode($row['id']) ?>" class="btn btn-danger" onclick="return confirm('Are you sure to delete your profile?')"><i class="fa fa-trash-o"></i></a>
                                </td>
                            </tr>
                            <?php
                        }
...
while ($row=mysqli_fetch_assoc($result)){
    $id=$row['id'];
    $lib_info=mysqli_query($con,"SELECT * FROM `libraian` WHERE `id`='$id'");
                                ...
                                <div class="col-md-12">
                                    <form method="post" action="">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" id="firstname" placeholder="firstname" value="<?=$lib_info_row['firstname'] ?>" required name="firstname">
                                            <input type="hidden" class="form-control" value="<?=$lib_info_row['id'] ?>" required name="id">
                                        </div>  
                                          <div class="form-group">
                                            <label for="lastname" >Last Name</label>
                                            <input type="text" class="form-control" id="lastname" placeholder="lastname" value="<?=$lib_info_row['lastname'] ?>" required name="lastname">
                                            <input type="hidden" class="form-control" value="<?=$lib_info_row['id'] ?>" required name="id">
                                        </div>
                                        <div class="form-group">
                                            <label for="email" >Email</label>
                                                <input type="text" class="form-control" id="email" placeholder="email" value="<?=$lib_info_row['email'] ?>"  required name="email">
                                        </div>
                                        <div class="form-group">
                                            <label for="username">Username</label>
                                                <input type="text" class="form-control" id="username" placeholder="username" value="<?=$lib_info_row['username'] ?>" required name="username">
                                        </div>
                                        <div class="form-group">
...
mysqli_query($con, "UPDATE `libraian` SET `firstname`='$firstname', `lastname`='$lastname', `email`='$email',`username`='$username', `password`='$password' WHERE `id`='$id'");
